...
export function WhatIsSection({
  ...
  const hasSecondParagraph = Boolean(secondParagraph?.trim());
  const hasCheckpoints = checkpoints.length > 0;
  const shouldShowLicenseTypes =
    sideContent === "licenseTypes" && Boolean(licenseTypes?.length);
  const shouldShowBenefits =
    sideContent === "benefitsBox" && Boolean(benefits?.length);
  const shouldShowSideContent = shouldShowLicenseTypes || shouldShowBenefits;
        ...
        >
            ...
            {hasCheckpoints && (
              <div className="space-y-4">
                {checkpoints.map((checkpoint) => (
                  <div key={checkpoint} className="flex items-center space-x-3">
          ...
          {shouldShowSideContent && (
            <div className="bg-secondary-gray p-8 rounded-2xl">
              {shouldShowLicenseTypes && licenseTypes ? (
                <>
